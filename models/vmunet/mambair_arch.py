...
# 实验表明，基于GroupConv的RFAConv性能更好，因为Unfold提取感受野空间特征时，一定程度上消耗时间比较严重。因此论文中选择了Group Conv的方法进行实验，并通过这种方式对CBAM和CA进行改进。
    ...
    def __init__(self, num_feat, is_light_sr= False, compress_ratio=3,squeeze_factor=30):
        ...
        if is_light_sr: # we use depth-wise conv for light-SR to achieve more efficient
            self.cab = nn.Sequential(
                ...
                # ECA(num_feat)
                RFAConv(in_channel=num_feat,out_channel=num_feat)
                # RFCAConv(inp=num_feat,oup=num_feat)
                # CBAM(channel=num_feat)
                # CoordAtt(inp=num_feat,oup=num_feat)
